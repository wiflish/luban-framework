...
public abstract class AbstractAliPayClient extends AbstractPayClient<AliPayClientConfig> {
    ...
    public AbstractAliPayClient(Long channelId, String channelCode, AliPayClientConfig config) {
    ...
    protected PayRefundRespDTO doUnifiedRefund(PayRefundUnifiedReqDTO reqDTO) throws AlipayApiException {
        ...
        if (Objects.equals(config.getMode(), AliPayClientConfig.MODE_CERTIFICATE)) {  // 证书模式
            response = client.certificateExecute(request);
        } else {
            response = client.execute(request);
        }
